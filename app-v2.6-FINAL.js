  ...
  parts.forEach((part, idx) => {
    ...
    if (match) {
      ...
      if (legendEntry) {
        ...
        // ОТЛАДКА: Логируем коды, которые не найдены в легенде
        console.warn(`⚠️ Code "${code}" not found in legend (from cell: "${cellValue}")`);
        
        // Пытаемся найти похожий код (с точкой или без)
        const codeWithDot = code + '.';
        const codeWithoutDot = code.replace(/\.$/, '');
        
        if (legend[codeWithDot]) {
          console.log(`✅ Found with dot: ${codeWithDot} -> ${legend[codeWithDot].short}`);
          results.push(`${legend[codeWithDot].short} / ${position}`);
        } else if (legend[codeWithoutDot] && codeWithoutDot !== code) {
          console.log(`✅ Found without dot: ${codeWithoutDot} -> ${legend[codeWithoutDot].short}`);
...
function getAvailableFilterValues(field) {
  ...
  if (field === 'position') {
    // ИСПРАВЛЕНО: Для должностей учитываем только те, которые ПРИНАДЛЕЖАТ выбранным подразделениям
    const selectedDepts = getDetailsSelectedCodes('filter-department');
    
    const positionMap = new Map(); // Должность -> Set(коды подразделений)
    
    filteredRows.forEach(row => {
      // Проходим по всем ролям в строке
      roleColumns.forEach(role => {
        const cellValue = row.roles[role];
        if (!cellValue) return;
        
        // Парсим ячейку: "ОПС / Архитектор" или "1.5.3 / Архитектор"
        ...
        parts.forEach(part => {
          ...
          if (match) {
            ...
            // Пытаемся найти код подразделения
            // Ищем в legendMap по сокращению
            let deptCode = null;
            const legendEntry = Object.values(legendMap).find(l => l.short === deptOrCode);
            if (legendEntry) {
              deptCode = legendEntry.code;
            } else {
              // Возможно, это код (1.5.3)
              deptCode = deptOrCode;
            }
            ...
            if (deptCode) {
              positionMap.get(position).add(deptCode);
            }
          }
        });
      });
    ...
    // Фильтруем: если выбраны подразделения, показываем только их должности
    if (selectedDepts && selectedDepts.length > 0) {
      const filteredPositions = [];
      positionMap.forEach((codes, position) => {
        // Проверяем, относится ли должность к выбранным подразделениям
        const belongsToDept = Array.from(codes).some(code => {
          return selectedDepts.some(filterCode => {
      ...
      // Если подразделения не выбраны, показываем все должности
