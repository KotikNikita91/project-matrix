...
function initTabs() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Поисковую строку показываем только на вкладке «Матрица»
      const searchRow = document.getElementById('search-bar-row');
      const filterRow = document.getElementById('controls-compact');
      if (searchRow) searchRow.style.display = tab === 'matrix' ? '' : 'none';
      if (filterRow) filterRow.parentElement.style.display = tab === 'matrix' ? '' : 'none';
...
/* ============================================================
   КОНСТРУКТОР ОТЧЁТА — «По должностям»
   Строки: функции (с фильтром по блоку)
   Колонки: выбранные должности
   Ячейки: роль (В, О, К, ...)
   ============================================================ */

let reportSelectedPositions = new Set(); // "ОтдКод / Должность"

/**
 * Строит индекс: для каждой строки матрицы — Map<positonKey, role>
 * где positionKey = "ДепШрт / ИмяДолжности" (как в display)
 */
function buildPositionRoleIndex() {
  // positionKey → Set<role>  (на случай если одна должность встречается в нескольких ролях)
  const index = new Map(); // rowIdx → Map<posKey, role>
...
/**
 * Все уникальные должности из данных, сгруппированные по подразделению.
 * Возвращает Map<deptKey, { short, full, color, positions: Set<posKey> }>
 */
function buildDeptPositionMap() {
  const depts = new Map();
  ...
  rawRows.forEach(row => {
    roleColumns.forEach(role => {
      ...
      value.split(' | ').forEach(seg => {
        ...
        // Определяем dept из сегмента
        const color = getColorFromSegment(seg);
        const shortMatch = seg.match(/^([^\s/]+)\s*\//);
        const deptShort = shortMatch ? shortMatch[1].trim() : '?';

        // Ищем полное название в легенде
        const entry = Object.values(legendMap).find(l => l.short === deptShort);
        const deptFull = entry ? entry.full : deptShort;
        const deptCode = entry ? getDepartmentCode(entry.code) : deptShort;
        const key = deptCode;

        if (!depts.has(key)) {
          depts.set(key, { short: deptShort, full: deptFull, color, positions: new Set() });
        }
        depts.get(key).positions.add(seg);
      });
    });
  });

  // Сортируем: числовые ключи по порядку
  const sorted = new Map([...depts.entries()].sort((a, b) => {
...
/** Заполняет конструктор — чекбоксы должностей по подразделениям */
function buildReportConstructor() {
  const picker = document.getElementById('report-dept-picker');
  if (!picker || rawRows.length === 0) return;
  picker.innerHTML = '';

  const deptMap = buildDeptPositionMap();

  deptMap.forEach((dept, deptKey) => {
    const group = document.createElement('div');
    group.className = 'report-dept-group';

    // Шапка группы
    const header = document.createElement('div');
    header.className = 'report-dept-group-header';
    const colorDot = document.createElement('div');
    colorDot.className = 'report-dept-color';
    colorDot.style.background = dept.color || '#ccc';
    const name = document.createElement('span');
    name.className = 'report-dept-group-name';
    name.textContent = `${dept.short} — ${dept.full}`;
    name.title = dept.full;
    const toggleAll = document.createElement('span');
    toggleAll.className = 'report-dept-toggle-all';
    toggleAll.textContent = 'все';
    ...
    posWrap.className = 'report-dept-positions';

    // Сортируем должности по алфавиту
    const sortedPos = [...dept.positions].sort((a, b) => a.localeCompare(b, 'ru'));

    sortedPos.forEach(posKey => {
      // Показываем только часть после "/"
      const posLabel = posKey.includes('/') ? posKey.split('/').slice(1).join('/').trim() : posKey;

      const item = document.createElement('div');
      item.className = 'report-pos-item';
      const cb = document.createElement('input');
      cb.type = 'checkbox';
      cb.id = 'rpos_' + hashString(posKey);
      ...
      posWrap.appendChild(item);
    });

    group.appendChild(posWrap);

    // «Все» для группы
    toggleAll.addEventListener('click', () => {
      const allChecked = sortedPos.every(p => reportSelectedPositions.has(p));
      sortedPos.forEach(p => {
        if (allChecked) reportSelectedPositions.delete(p);
        else reportSelectedPositions.add(p);
      });
      posWrap.querySelectorAll('input[type=checkbox]').forEach(cb => {
...
function buildReportBlockFilter() {
  ...
  const cur = sel.value;
  sel.innerHTML = '<option value="">Все блоки</option>';
...
function updateReportColCount() {
  ...
  if (el) el.textContent = `Выбрано должностей: ${reportSelectedPositions.size}`;
}

/** Основной рендер таблицы отчёта */
function renderReport() {
  ...
  if (reportSelectedPositions.size === 0) {
    tableEl.innerHTML = `<tr><td colspan="3" class="report-empty">
      👆 Выберите подразделения или должности выше — они станут столбцами таблицы
    </td></tr>`;
    return;
  }
  ...
  // Строим индекс posKey→role для каждой строки
  const posIndex = buildPositionRoleIndex();

  // Отсортированный список выбранных должностей
  const selPos = [...reportSelectedPositions].sort((a, b) => a.localeCompare(b, 'ru'));

  // Шапка
  const thead = document.createElement('thead');
  const hrow = document.createElement('tr');
  ['№', 'Блок', 'Функция'].forEach((t, i) => {
    const th = document.createElement('th');
    th.textContent = t;
    th.className = i === 2 ? 'col-func-header' : '';
    hrow.appendChild(th);
  });
  selPos.forEach(pos => {
    const th = document.createElement('th');
    th.className = 'col-pos-header';
    // Показываем только часть после "/"
    const label = pos.includes('/') ? pos.split('/').slice(1).join('/').trim() : pos;
    th.textContent = label;
    th.title = pos;
    // Цвет шапки = цвет подразделения
    const bg = getColorFromSegment(pos);
    if (bg) {
      th.style.background = bg;
      th.style.color = '#333';
      th.style.borderColor = 'rgba(0,0,0,0.1)';
    }
    hrow.appendChild(th);
  });
  ...
  rows.forEach((row, ri) => {
    const rowMap = posIndex.get(rawRows.indexOf(row)) || new Map();

    // Проверяем — есть ли хоть одна совпадающая должность
    const hasAny = selPos.some(p => rowMap.has(p));
    if (!hasAny) return; // Пропускаем строки без совпадений

    const tr = document.createElement('tr');

    // Фиксированные колонки
    [row.num, row.block, row.func].forEach((val, ci) => {
      const td = document.createElement('td');
      td.textContent = String(val ?? '');
      if (ci === 0) td.className = 'cell-num';
      else if (ci === 1) td.className = 'cell-block';
      else td.className = 'cell-func';
      tr.appendChild(td);
    });
    ...
    selPos.forEach(pos => {
      ...
      if (role) {
        ...
        badge.className = `role-badge role-${role.replace(/[^А-ЯЁA-Z]/g, '') || 'default'}`;
  ...
  if (renderedRows === 0) {
    tbody.innerHTML = `<tr><td colspan="${3 + selPos.length}" class="report-empty">
      Нет строк с выбранными должностями
    </td></tr>`;
  }
...
function initReportExport() {
  ...
  document.getElementById('report-select-all')?.addEventListener('click', () => {
    document.querySelectorAll('#report-dept-picker input[type=checkbox]').forEach(cb => {
      cb.checked = true;
...
function exportReportExcel() {
  const tableEl = document.getElementById('report-table');
  if (!tableEl || !tableEl.querySelector('tbody tr td:not(.report-empty)')) {
    showToast('Нет данных для экспорта', 1200); return;
  }
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.table_to_sheet(tableEl);
  XLSX.utils.book_append_sheet(wb, ws, 'По должностям');
  const now = new Date();
  XLSX.writeFile(wb, `Матрица_по_должностям_${now.getDate().toString().padStart(2,'0')}.${(now.getMonth()+1).toString().padStart(2,'0')}.${now.getFullYear()}.xlsx`);
...
function exportReportPDF() {
  ...
  if (rows.length < 2) { showToast('Нет данных', 1200); return; }
...
<title>Матрица_по_должностям_${date}</title>
<style>
@page { size: A3 landscape; margin: 10mm; }
* { box-sizing:border-box; margin:0; padding:0; }
body { font-family:Arial,sans-serif; font-size:8pt; }
h1 { font-size:12pt; color:#6b4eff; margin-bottom:8px; }
.date { font-size:8pt; color:#888; float:right; margin-top:-22px; }
table { width:100%; border-collapse:collapse; }
th { background:#6b4eff; color:#fff; padding:4px 3px; font-size:7pt; border:1px solid #5a3ee0; text-align:center; }
th.func { background:#3d2ab0; text-align:left; }
td { padding:4px 3px; border:1px solid #ddd; font-size:7pt; vertical-align:top; }
tr:nth-child(even) td { background:#fafafa; }
@media print { body { -webkit-print-color-adjust:exact; print-color-adjust:exact; } }
</style></head><body>
<h1>Функциональная матрица — по должностям</h1>
<div class="date">${date}</div>
<table><thead><tr>${headers.map((h,i) => `<th${i===2?' class="func"':''}>${h}</th>`).join('')}</tr></thead>
<tbody>${bodyRows.map(r => `<tr>${r.map((c,i) => `<td>${c}</td>`).join('')}</tr>`).join('')}</tbody>
</table>
...
// Инициализируем экспорт и конструктор после загрузки данных
