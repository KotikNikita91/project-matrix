/* app.js — Функциональная матрица v2.2 (заголовки из первой строки) */
...
function parseMatrix(data, legend) {
  ...
  console.log('📊 Matrix parsing:');
  console.log('- Total rows:', rows.length);
  
  // ВАЖНО: Берем заголовки из ПЕРВОЙ СТРОКИ данных
  ...
  for (let i = roleStartIndex; i < headers.length; i++) {
    ...
    if (role) {
      ...
      console.log(`    ⚠️ Could not extract role`);
    }
  }
  ...
  // Парсим данные со 2-й строки (индекс 1)
...
function buildTable() {
  ...
  console.log('🏗️ Building table with', roleColumns.length, 'role columns');
