...
function onExportPDF() {
  ...
  try {
    ...
    pdf.text('Funkcionalnaya matrica', margin, margin + 4);
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(120);
    pdf.text(new Date().toLocaleDateString('ru-RU'), pdfW - margin - 20, margin + 4);
    pdf.setTextColor(0);

    // Заголовки таблицы (транслитерируем кириллицу для совместимости)
    const colHeaders = [
      { header: '#', dataKey: 'num' },
      { header: 'Blok', dataKey: 'block' },
      { header: 'Funkciya', dataKey: 'func' },
      { header: 'Produkt', dataKey: 'product' },
      ...roleColumns.map(r => ({ header: r, dataKey: `role_${r}` }))
    ];

    // Данные
    const tableData = lastRenderedRows.map(r => {
      const row = {
        num: String(r.num || ''),
        block: translitShort(String(r.block || '')),
        func: translitShort(String(r.func || '')),
        product: translitShort(String(r.product || ''))
      };
      roleColumns.forEach(role => {
        row[`role_${role}`] = translitShort(String(r.roles[role] || ''));
      });
      return row;
    });

    // Ширины колонок (мм) — подстраиваем под A3
    const totalW = pdfW - margin * 2;
    const fixedW = 10 + 40 + 60 + 45; // №, Блок, Функция, Продукт
    const roleW = Math.max(12, Math.floor((totalW - fixedW) / Math.max(roleColumns.length, 1)));
    const funcW = Math.max(50, totalW - 10 - 40 - 45 - roleW * roleColumns.length);

    const colStyles = {
      num:     { cellWidth: 10, fontSize: 7 },
      block:   { cellWidth: 40, fontSize: 7 },
      func:    { cellWidth: funcW, fontSize: 7 },
      product: { cellWidth: 45, fontSize: 7 }
    };
    roleColumns.forEach(r => {
      colStyles[`role_${r}`] = { cellWidth: roleW, fontSize: 6, halign: 'left' };
    });

    // Цвета строк по блокам (берём из DEPARTMENT_COLORS по первой цифре блока)
    const deptColorCache = {};
    function getRowFillColor(rowData) {
      const blockStr = String(rowData.block || '');
      const key = blockStr.match(/^\d+/)?.[0] ?? 'x';
      if (!(key in deptColorCache)) {
        const hex = DEPARTMENT_COLORS[key] || DEPARTMENT_COLORS.default;
        deptColorCache[key] = hexToRgb(hex);
      }
      return deptColorCache[key];
    }
    ...
    pdf.autoTable({
      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          const fill = getRowFillColor(data.row.raw);
      ...
      didDrawPage: (data) => {
        ...
        pdf.text(`${pageNum}`, pdfW - margin, pdf.internal.pageSize.getHeight() - 4);
        pdf.setTextColor(0);
      }
    });
...
/** Упрощённая транслитерация для PDF (jsPDF не поддерживает Unicode без встроенных шрифтов) */
function translitShort(s) {
  if (!s) return '';
  const map = {
...
/** Конвертация HEX цвета в RGB массив [r, g, b] */
