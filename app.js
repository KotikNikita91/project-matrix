...
function renderTable() {
  ...
  tbody.innerHTML = rows.map(row => {
    ...
    roleColumns.forEach(role => {
      ...
      // Цвет красим только под текстом (не всю ячейку)
      function wrapWithBg(text) {
        if (!bgColor || !text) return text;
        return `<span class="pos-bg" style="background:${bgColor}">${text}</span>`;
      }

      let displayHtml;
      if (value.includes(' | ')) {
        displayHtml = value.split(' | ').map((v, i) => {
          const entry = i === 0
            ? `<span class="pos-entry">${escapeHtml(v.trim())}</span>`
            : `<span class="pos-sep">·</span><span class="pos-entry">${escapeHtml(v.trim())}</span>`;
          return wrapWithBg(entry);
        }).join('');
      } else {
        displayHtml = value ? wrapWithBg(`<span class="pos-entry">${escapeHtml(value)}</span>`) : '';
      }
  ...
  const tbody = lastRenderedRows.map(row => {
    const cells = colDefs.map(c => {
      if (c.role !== undefined) {
        ...
        const code  = row.roleCodes[c.role] || '';
        const bg    = getCellColor(code);
        ...
        const cellHtml = value.includes(' | ')
          ? value.split(' | ').map((v, i) => {
              const inner = `<span${bgStyle}>${escapeHtml(v.trim())}</span>`;
              return i === 0
                ? `<span class="pos-entry">${inner}</span>`
                : `<span class="pos-sep">·</span><span class="pos-entry">${inner}</span>`;
            }).join('')
          : (value ? `<span${bgStyle}>${escapeHtml(value)}</span>` : '');
