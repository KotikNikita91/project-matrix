 ...
 */
/** Нормализация ключа позиции для сравнения (убирает лишние пробелы, приводит к нижнему регистру) */
function normPos(s) {
...
function buildDeptHierarchy() {
  const top = new Map();
  const posIdx = buildPositionRoleIndex();

  // Проходим ПО posIndex — берём оригинальные строки
  posIdx.forEach(({ origMap }) => {
    origMap.forEach((origSeg) => {
      if (!origSeg) return;

      // Определяем dept из оригинальной строки
      const shortMatch = origSeg.match(/^([^\s/]+)\s*\//);
      if (!shortMatch) {
        // Нет слеша — помещаем в группу "Прочее"
        const key = '__other__';
        if (!top.has(key)) {
          top.set(key, { short: '?', full: 'Прочее', color: '#e0e0e0',
            children: new Map(), positions: new Set(), sortKey: 9999 });
        }
        top.get(key).positions.add(origSeg);
        return;
      }
      ...
      const code     = entry ? entry.code : deptShort;
      const topCode  = code.split('.')[0];
      const topEntry = legendMap[topCode] || Object.values(legendMap).find(l => l.code === topCode);
      const topShort = topEntry ? topEntry.short : (entry ? deptShort : topCode);
      const topFull  = topEntry ? topEntry.full  : (entry ? entry.full : deptShort);
      ...
      if (!entry || code.split('.').length === 1) {
        // Прямая должность верхнего уровня
        topDept.positions.add(origSeg);
      } else {
        // Дочернее подразделение
        const subCode  = code.split('.').slice(0, 2).join('.');
        ...
        if (subEntry && !topDept.children.has(subCode)) {
          topDept.children.set(subCode, {
            ...
            color, positions: new Set(),
            sortKey: parseFloat(subCode) || 0
          });
        }
        if (topDept.children.has(subCode)) {
          topDept.children.get(subCode).positions.add(origSeg);
        } else {
          topDept.positions.add(origSeg);
        }
      }
  ...
  newSel.addEventListener('change', () => {
    ...
    if (selectedBlock) {
      ...
      blockRows.forEach(row => {
        ...
        const entry = posIndex.get(ri);
        if (!entry) return;
        // Собираем оригинальные строки позиций через origMap
        entry.origMap.forEach((origSeg) => blockPositions.add(origSeg));
      });
