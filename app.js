...
async function fetchGoogleSheet(url) {
  ...
  if (!response.ok) throw new Error('Не удалось загрузить данные. Проверьте доступ к таблице.');
  const text = await response.text();
  const jsonMatch = text.match(/google\.visualization\.Query\.setResponse\(([\s\S]*)\);?/);
  if (!jsonMatch) throw new Error('Неверный формат ответа от Google Sheets');
...
function parseCellValue(cellValue, legend) {
  ...
  parts.forEach((part, idx) => {
    ...
    if (match) {
      ...
      if (legendEntry) {
        ...
        const codeWithDot = code + '.';
        const codeWithoutDot = code.replace(/\.$/, '');
        if (legend[codeWithDot]) {
          results.push(`${legend[codeWithDot].short} / ${position}`);
        } else if (legend[codeWithoutDot] && codeWithoutDot !== code) {
          results.push(`${legend[codeWithoutDot].short} / ${position}`);
        } else {
  ...
  return { display: results.join(', '), code: firstCode, positions };
}
...
/* ==================== БЛОК ФУНКЦИЙ — ТОЛЬКО ВЕРХНИЙ УРОВЕНЬ ==================== */

/**
 * Извлекает верхнеуровневый код блока.
 * "1.2. Проектирование" → "1"
 * "0. Общий"            → "0"
 * "12"                  → "12"
 */
function getTopLevelBlock(blockVal) {
  if (!blockVal) return '';
  const s = String(blockVal).trim();
  const match = s.match(/^(\d+)/);
  return match ? match[1] : s;
}

/**
 * Проверяет, принадлежит ли блок строки одному из выбранных верхнеуровневых блоков.
 */
function blockMatchesFilter(rowBlock, selectedTopBlocks) {
  if (!selectedTopBlocks || selectedTopBlocks.length === 0) return true;
  const rowTop = getTopLevelBlock(rowBlock);
  return selectedTopBlocks.includes(rowTop);
}
...
function getAvailableFilterValues(field) {
  ...
  if (field === 'block') {
    // Только верхнеуровневые блоки (целые числа: 0, 1, 2, 3...)
    const topBlockSet = new Set();
    filteredRows.forEach(r => {
      if (!r.block) return;
      topBlockSet.add(getTopLevelBlock(r.block));
    });
    vals = Array.from(topBlockSet)
      .filter(Boolean)
      .sort((a, b) => {
...
function getFilteredRowsExcluding(excludeField) {
  ...
  FILTER_CONFIG.forEach(cfg => {
    ...
    } else if (cfg.field === 'block') {
      filters[cfg.field] = getDetailsSelectedValues(cfg.id); // верхнеуровневые
    } else {
...
function applyFilters(row, filters) {
  if (filters.block && filters.block.length > 0) {
    if (!blockMatchesFilter(row.block, filters.block)) return false;
  }
...
function onExportExcel() {
  ...
  XLSX.writeFile(wb, `functional-matrix-${ts}.xlsx`);
...
async function onExportPDF() {
  ...
  showToast('Генерация PDF, подождите...', 8000);
  ...
  try {
    ...
    const origScrollLeft = tableEl.scrollLeft;
    const origScrollTop = tableEl.scrollTop;
    tableEl.scrollLeft = 0;
    tableEl.scrollTop = 0;

    // Рендерим всю таблицу целиком
    ...
    tableEl.scrollLeft = origScrollLeft;
    tableEl.scrollTop = origScrollTop;

    const imgWidth = canvas.width;
    const imgHeight = canvas.height;

    // PDF — A3 горизонтальный (420 × 297 мм)
    ...
    pdf.text('Funktsionalnaya matritsa', margin, margin + 5); // кириллица → транслит в заголовке
    pdf.setFontSize(8);
    pdf.setFont('helvetica', 'normal');
    pdf.text(new Date().toLocaleDateString('ru-RU'), pdfW - margin - 28, margin + 5);

    const availW = pdfW - margin * 2;
    const availH = pdfH - margin * 2 - headerH;
    const scale = availW / imgWidth;
    const scaledTotalH = imgHeight * scale;

    if (scaledTotalH <= availH) {
      // Всё умещается на одной странице
      const imgData = canvas.toDataURL('image/jpeg', 0.88);
      pdf.addImage(imgData, 'JPEG', margin, margin + headerH, availW, scaledTotalH);
    } else {
      // Разбиваем на страницы
      const pageHeightPx = availH / scale; // высота одной страницы в пикселях canvas'а
      let sourceY = 0;
      let pageNum = 1;
      ...
      while (sourceY < imgHeight) {
        ...
        const sliceHeightPx = Math.min(pageHeightPx, imgHeight - sourceY);

        // Создаём холст для среза
        ...
        const sliceRenderedH = sliceHeightPx * scale;

        const yStart = pageNum === 1 ? margin + headerH : margin;
        pdf.addImage(sliceData, 'JPEG', margin, yStart, availW, sliceRenderedH);

        // Номер страницы
        pdf.setFontSize(7);
        pdf.setTextColor(150);
        pdf.text(`${pageNum}`, pdfW - margin - 4, pdfH - margin);
        pdf.setTextColor(0);
    ...
    const ts = new Date().toISOString().replace(/[:\-]/g, '').split('.')[0];
    pdf.save(`functional-matrix-${ts}.pdf`);
    showToast('PDF сохранён ✓', 2000);
  } catch (err) {
