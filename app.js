...
function initTabs() {
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (controlsRow) controlsRow.closest('.header-right').querySelector('.controls-compact').style.display = tab === 'matrix' ? '' : 'none';
...
function buildReportBlockFilter() {
  ...
  if (!sel) return;
  const cur = sel.value;
  // Remove existing listener by replacing element
  ...
  newSel.addEventListener('change', renderReport);
}
...
function renderReport() {
  ...
  selPos.forEach(pos => {
    const th = document.createElement('th');
    th.className = 'rth-pos';
    // Заголовок = сокращение отдела + должность
    ...
    th.innerHTML = deptShort
      ? `<span class="rth-dept-badge">${escapeHtml(deptShort)}</span>${escapeHtml(posLabel)}`
      : escapeHtml(posLabel);
    th.title = pos;
...
function initReportLegend() {
  const btn = document.getElementById('report-legend-btn');
  if (!btn) return;
  btn.addEventListener('click', openLegend);  // переиспользуем основную легенду
}
...
function initVisitCounter() {
  ...
  // Увеличиваем счётчик в localStorage (личный, текущий браузер)
  const myKey = 'matrix_my_visits';
  const myVisits = (parseInt(localStorage.getItem(myKey) || '0') + 1);
  localStorage.setItem(myKey, myVisits);

  // Если есть GAS_PROXY_URL — используем GAS как общий счётчик
  if (GAS_PROXY_URL) {
    const counterUrl = GAS_PROXY_URL + '?action=counter';
    fetch(counterUrl)
      .then(r => r.json())
      .then(d => {
        if (d && d.count !== undefined) {
          el.textContent = `👁 ${d.count.toLocaleString('ru')}`;
          el.title = `Всего посещений: ${d.count}`;
        }
      })
      .catch(() => {
        el.textContent = `👁 ${myVisits}`;
        el.title = 'Мои посещения (локально)';
      });
  } else {
    el.textContent = `👁 ${myVisits}`;
